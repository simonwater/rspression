...
impl PrefixParselet for IfParselet {
    fn parse(&self, parser: &mut Parser, _token: &Token) -> LoxResult<Expr> {
        parser.consume(crate::token::TokenType::LeftParen, "Expected '(' after 'if'")?;
        let condition = parser.expression_prec(Precedence::PREC_NONE)?;
        parser.consume(crate::token::TokenType::Comma, "Expected ',' after condition")?;
        let then_branch = parser.expression_prec(Precedence::PREC_NONE)?;
        parser.consume(crate::token::TokenType::Comma, "Expected ',' after then branch")?;
        let else_branch = parser.expression_prec(Precedence::PREC_NONE)?;
        parser.consume(crate::token::TokenType::RightParen, "Expected ')' after else branch")?;
