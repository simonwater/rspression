...
impl VM {
    ...
    fn read_byte(&mut self) -> LoxResult<u8> {
        let chunk = self.chunk.as_ref().ok_or_else(|| crate::error::LoxError::RuntimeError {
            message: "No chunk loaded".to_string(),
        })?;
        
        let byte = chunk.read_byte(self.ip).ok_or_else(|| crate::error::LoxError::RuntimeError {
    ...
    fn get_constant(&self, index: usize) -> LoxResult<&Value> {
        ...
        chunk.read_constant(index).ok_or_else(|| crate::error::LoxError::RuntimeError {
    ...
    fn pop(&mut self) -> LoxResult<Value> {
        self.stack.pop().ok_or_else(|| crate::error::LoxError::RuntimeError {
            message: "Stack underflow".to_string(),
        })
    }
    
    fn peek(&self) -> LoxResult<&Value> {
        self.stack.last().ok_or_else(|| crate::error::LoxError::RuntimeError {
            message: "Stack underflow".to_string(),
        })
    }
